...
const debugHudSettings = [
 ['conductor','Optional explicit conductor reference.','Opsiyonel açık conductor referansı.','Leave empty to auto-bind to the active BST conductor at runtime.','Runtime’da aktif BST conductor otomatik bulunsun istiyorsan boş bırak.'],
 ['stateText','Text element showing the current state.','Mevcut state’i gösteren Text elementi.','Use it to see Explore, Combat, Boss or NO CONDUCTOR while testing.','Testte Explore, Combat, Boss veya NO CONDUCTOR görmek için kullan.'],
 ['barBeatText','Text element showing current bar and beat.','Mevcut bar ve beat değerini gösteren Text elementi.','This is the musical clock; use it to verify quantized changes land correctly.','Bu müzikal saattir; quantized değişimler doğru yere oturuyor mu kontrol etmek için kullan.'],
 ['bpmText','Text element showing current BPM.','Mevcut BPM’i gösteren Text elementi.','Useful when sections override tempo or a track feels off-grid.','Section tempo override ettiğinde veya track grid dışı hissedince kullanışlıdır.'],
 ['intensityText','Text element showing numeric intensity.','Sayısal intensity değerini gösteren Text elementi.','Confirms gameplay is really driving the 0-1 value you expect.','Gameplay beklediğin 0-1 değeri gerçekten sürüyor mu doğrular.'],
 ['intensitySlider','Slider visualizing current intensity.','Mevcut intensity değerini görselleştiren Slider.','Good for designers because energy changes are visible without reading numbers.','Designer için iyidir; enerji değişimi sayı okumadan görünür.'],
 ['layersText','Text element listing active layers.','Aktif layer listesini gösteren Text elementi.','Use it to debug manual, intensity and zone layer ownership.','Manual, intensity ve zone layer sahipliğini debug etmek için kullan.'],
 ['activeZoneText','Text element showing the active/top zone.','Aktif/üst zone’u gösteren Text elementi.','Use it when overlapping zones make ownership unclear.','Üst üste zonelar sahipliği belirsizleştiriyorsa kullan.'],
 ['noConductorColor','HUD color when no conductor is available.','Conductor yokken kullanılan HUD rengi.','A warning color makes setup failures obvious.','Uyarı rengi kurulum hatasını belirgin yapar.'],
 ['exploreColor / combatColor / calmColor / tensionColor','Built-in state color mapping.','Hazır state renk eşlemesi.','Map common states to readable colors for quick QA scanning.','Yaygın stateleri hızlı QA taraması için okunur renklere bağlar.'],
 ['defaultStateColor','Fallback color for all other state names.','Diğer tüm state adları için fallback renk.','Use when your project has custom names such as Boss, Menu or Chase.','Boss, Menu veya Chase gibi özel isimlerde kullan.'],
 ['refreshInterval','Seconds between HUD refreshes.','HUD güncellemeleri arasındaki saniye.','Lower feels more live; higher costs less UI work.','Düşük daha canlı hisseder; yüksek daha az UI işi yapar.']
];
...
const panelSettings = [
 ...
 ['stateSlots','Quick state buttons.','Hızlı state butonları.','Add your real gameplay state names.','Gerçek gameplay state adlarını ekle.'],
...
panelSettings.push(
 ['lockCursorWhenHidden','Locks and hides the cursor while the panel is hidden.','Panel gizliyken cursoru kilitler ve gizler.','Turn off if your own input system owns cursor state.','Cursor durumunu kendi input sistemin yonetiyorsa kapat.'],
 ['panelRect','Window rectangle used by compact layout.','Kompakt layout tarafindan kullanilan pencere alani.','Use only when fullscreen layout is off.','Sadece fullscreen layout kapaliyken kullan.'],
 ['drawOpaqueBackdrop','Draws a dark backdrop behind fullscreen controls.','Tam ekran kontrollerin arkasina koyu zemin cizer.','Keep on for readability in bright scenes.','Parlak sahnelerde okunurluk icin acik tut.'],
 ['startStateName','State used by the panel Start Music button.','Panel Start Music butonunun kullandigi state.','Leave empty to let the conductor choose its normal start state.','Conductor normal baslangic stateini secsin istiyorsan bos birak.'],
 ['stateTransitionMode','Transition mode used by panel state buttons.','Panel state butonlarinin kullandigi transition modu.','Use OnNextTransitionPoint to test authored section exits during state changes.','State degisimlerinde yazilmis section exitlerini test etmek icin OnNextTransitionPoint kullan.'],
 ['trackTransitionMode','Transition mode used by panel track jump buttons.','Panel track jump butonlarinin kullandigi transition modu.','Use OnSectionEnd or OnNextTransitionPoint when jumping from a section track to another track in the same state.','Section trackten ayni state icinde baska tracke atlarken OnSectionEnd veya OnNextTransitionPoint kullan.'],
 ['stateCrossfadeDuration / trackCrossfadeDuration','Panel crossfade overrides for state and track changes.','Panel state ve track degisimleri icin crossfade override degerleri.','Use -1 to inherit conductor defaultCrossfadeDuration; use 0 for a hard cut; use explicit seconds to test OnSectionEnd pre-roll overlap.','-1 conductor defaultCrossfadeDuration miras alir; 0 sert kesit yapar; acik saniye degeri OnSectionEnd pre-roll overlap testinde kullanilir.'],
 ['startFadeDuration / stopFadeDuration','Panel fade overrides for start and stop.','Panel baslatma/durdurma fade override degerleri.','-1 inherits conductor start/stop fade defaults.','-1 conductor start/stop fade varsayilanlarini miras alir.'],
 ['forceStateWhileInZone','Panel state buttons can beat an active zone.','Panel state butonlari aktif bolgeyi gecebilir.','Use only when testing scripted ownership.','Sadece scripted sahiplik testinde kullan.'],
 ['persistStateAfterLeavingZone','Stores the chosen state as post-zone fallback.','Secilen statei bolge sonrasi fallback olarak saklar.','Keep on for normal state buttons.','Normal state butonlari icin acik tut.'],
 ['stateStartTrackIndex / stateStartSectionIndex','Optional track/section target for panel state changes.','Panel state degisimleri icin opsiyonel track/section hedefi.','-1 means default track or the track entry section.','-1 varsayilan track veya track entry section demektir.'],
 ['quantizeLayerChanges','Panel layer changes wait for the bar.','Panel layer degisimleri bari bekler.','Keep on when auditioning real gameplay feel.','Gercek gameplay hissini denerken acik tut.'],
 ['layerFadeInOverride / layerFadeOutOverride','Panel layer fade overrides.','Panel layer fade override degerleri.','-1 uses the layer/conductor fade rules.','-1 layer/conductor fade kurallarini kullanir.'],
 ['queueSectionChangesToValidExit','Panel section jumps wait for valid exits.','Panel section atlamalari gecerli cikisi bekler.','This mirrors musical section authoring.','Muzikal section authoring davranisini yansitir.'],
 ['crossfadeSectionChanges / sectionCrossfadeDuration','Panel section jump crossfade controls.','Panel section atlama crossfade kontrolleri.','Use -1 duration to inherit conductor crossfade.','Sureyi -1 birakirsan conductor crossfade miras alinir.'],
 ['quantizeIntensityChanges','Panel intensity changes wait for the bar.','Panel intensity degisimleri bari bekler.','Use to hear energy changes musically.','Enerji degisimlerini muziksel duymak icin kullan.'],
 ['smoothIntensityChanges','Panel intensity changes glide toward the target.','Panel intensity degisimleri hedefe dogru yumusak akar.','When used with quantize, smoothing starts at the next bar.','Quantize ile birlikte kullanilinca smoothing sonraki barda baslar.'],
 ['intensitySmoothSpeed / intensitySmoothEasing','Panel smooth intensity shape.','Panel smooth intensity hizi ve egrisi.','Use 0 speed to inherit the conductor smoothIntensitySpeed.','0 hiz conductor smoothIntensitySpeed degerini miras alir.'],
 ['intensityLayerFadeOverride','Panel-only intensity layer fade override.','Sadece panel icin intensity layer fade override degeri.','Use 0 to verify smooth is truly off; use -1 to hear authored/conductor fades.','Smooth gercekten kapali mi test etmek icin 0; authored/conductor fade icin -1 kullan.'],
 ['requestId / requestState / requestPriority','Single request editor fields.','Tek request duzenleme alanlari.','Useful for quickly pushing one test request.','Tek test requestini hizli itmek icin kullanisli.'],
 ['quantizeRequests','Panel request switches use musical timing.','Panel request gecisleri muzik zamanlama kullanir.','Turn off only for hard debug cuts.','Sadece sert debug kesitlerinde kapat.'],
 ['requestSlots','Reusable no-code request test rows.','Yeniden kullanilabilir kodsuz request test satirlari.','Model combat, low-health and cutscene ownership before wiring code.','Koda baglamadan combat, dusuk can ve cutscene sahipligini modelle.'],
 ['duckTargetVolume / duckFadeTime','Panel ducking test values.','Panel ducking test degerleri.','Audition dialogue dips without writing gameplay.','Gameplay yazmadan diyalog kisma davranisini dene.'],
 ['Duck layers / Duck stingers','Panel toggles for what ducking affects.','Duckingin neyi etkileyecegini secen panel togglelari.','Stingers stay in front by default unless Duck stingers is enabled.','Duck stingers acilmazsa stingerlar varsayilan olarak onde kalir.']
);

const fadeInheritanceRows = [
 ['StartMusic(fadeDuration: -1)','Uses conductor startFadeDuration.','Conductor startFadeDuration kullanir.','Start fades are transport fades, not crossfades.','Baslatma fadeleri transport fadeidir, crossfade degildir.'],
 ['StopMusic(fadeDuration: -1)','Uses conductor stopFadeDuration.','Conductor stopFadeDuration kullanir.','Use explicit 0 for hard stops.','Sert durus icin acikca 0 ver.'],
 ['State / track / section crossfade -1','Uses conductor defaultCrossfadeDuration.','Conductor defaultCrossfadeDuration kullanir.','Override per call for special moments.','Ozel anlar icin cagri bazinda override et.'],
 ['Layer fade -1','Uses layer fade if set, otherwise defaultCrossfadeDuration.','Layer fade ayarliysa onu, degilse defaultCrossfadeDuration kullanir.','Author per-stem fades for drums, pads and impacts.','Davul, pad ve impact icin stem basina fade yaz.'],
 ['Stinger fadeIn -1','Uses conductor defaultStingerFadeIn.','Conductor defaultStingerFadeIn kullanir.','This does not use the music crossfade.','Bu muzik crossfade ayarini kullanmaz.'],
 ['Ducking fadeTime -1','Uses duckingFadeIn or duckingFadeOut.','duckingFadeIn veya duckingFadeOut kullanir.','Start and stop can feel different.','Baslama ve bitirme farkli hissedebilir.']
];

const stateParameterRows = [
 ['transitionMode','Immediate, OnNextBar, OnSectionEnd, or OnNextTransitionPoint.','Immediate, OnNextBar, OnSectionEnd veya OnNextTransitionPoint.','Choose when the state change is allowed to land.','State degisiminin ne zaman oturacagini secer.'],
 ['crossfadeDuration','State transition crossfade override; -1 inherits defaultCrossfadeDuration.','State gecis crossfade override degeri; -1 defaultCrossfadeDuration miras alir.','Use 0 for a hard cut, short for hits, longer for pads.','Sert kesit icin 0, vurgu icin kisa, pad icin uzun kullan.'],
 ['forceWhileInZone','Allows a state call to beat the active music zone.','State cagrisinin aktif muzik bolgesini gecmesine izin verir.','Use for cutscenes or moments that must own the score.','Skoru kesin sahiplenmesi gereken cutscene/anlar icin kullan.'],
 ['persistAfterLeavingZone','Stores this state as the global fallback after zones end.','Bolgeler bitince global fallback olarak bu statei saklar.','Keep true for normal gameplay state changes.','Normal gameplay state degisimlerinde true tut.'],
 ['startTrackIndex','Optional track index inside the target state; -1 uses state defaults.','Hedef state icinde opsiyonel track indexi; -1 state varsayilanlarini kullanir.','Use when a state must begin on a specific cue.','State belirli cue ile baslamaliysa kullan.'],
 ['startSectionIndex','Optional starting section for section tracks; -1 uses entrySectionIndex.','Section trackler icin opsiyonel baslangic sectioni; -1 entrySectionIndex kullanir.','Useful for skipping an intro or entering a phase directly.','Intro atlamak veya faza direkt girmek icin kullanisli.']
];
...
const conditionFieldRows = [
 ...
 ['StateEquals / StateNotEquals','targetStateName','targetStateName','Compares the current state name exactly.','Mevcut state adini birebir karsilastirir.'],
 ['IntensityAbove / IntensityBelow','intensityThreshold','intensityThreshold','Above uses > and Below uses <.','Above >, Below < kullanir.'],
 ...
 ['CustomFlag','flagName + expectedFlagValue','flagName + expectedFlagValue','Reads a boolean set by BST.SetCustomFlag.','BST.SetCustomFlag ile ayarlanan booleani okur.'],
 ['LoopCountAbove','loopCountThreshold','loopCountThreshold','Passes only when current loop count is greater than the threshold.','Sadece mevcut loop sayisi threshold degerinden buyukse gecer.'],
 ['Always','none','yok','Always passes; use as the final fallback.','Her zaman gecer; son fallback olarak kullan.']
];

const defaultLayerModeRows = [
 ['MutedOnly','Prepared layers play at standby volume until enabled.','Hazirlanan layerlar acilana kadar standby volume seviyesinde calar.','Best default for adaptive stem music.','Adaptif stem muzigi icin en iyi varsayilan.'],
 ['AllAvailable','All available layers start audible.','Tum mevcut layerlar duyulur baslar.','Use for full arrangement starts.','Dolu aranjman baslangiclari icin kullan.'],
 ['CustomList','Only trackDefaultLayers start audible.','Sadece trackDefaultLayers duyulur baslar.','Use for a curated starting mix.','Secilmis baslangic miksi icin kullan.'],
 ['UseConductorDefault','Track uses conductor defaultLayerMode and defaultActiveLayers.','Track conductor defaultLayerMode ve defaultActiveLayers kullanir.','Good for shared/global arrangements.','Ortak/global aranjmanlar icin iyi.'],
 ['InheritFromPrevious','Legacy value normalized to MutedOnly during validation.','Legacy deger validation sirasinda MutedOnly olur.','Do not build new authoring around it.','Yeni authoring icin buna dayanma.']
];

const getterDefaultRows = [
 ['GetMusicStateName','Returns current state or null.','Mevcut statei veya null dondurur.','Null means no conductor/state has resolved yet.','Null, henuz conductor/state cozulemedi demektir.'],
 ['GetCurrentTrackIndex / GetCurrentSectionIndex / GetPendingSectionIndex','Return -1 when unavailable.','Yoksa -1 dondurur.','Treat -1 as no current target.','-1 degerini mevcut hedef yok diye yorumla.'],
 ['GetCurrentSectionBar','Returns the current section-local bar, or 0 when sections are inactive.','Mevcut section-local bar degerini dondurur; section aktif degilse 0.','Use this for validExitBars debugging; GetCurrentBar is the global transport bar.','validExitBars debug icin bunu kullan; GetCurrentBar global transport baridir.'],
 ['GetCurrentTrackName / GetCurrentSectionName','Return an empty string when unavailable.','Yoksa bos string dondurur.','Safe for labels and debug UI.','Label ve debug UI icin guvenlidir.'],
 ['List getters','Return empty lists when unavailable.','Yoksa bos liste dondurur.','Iterate without null checks.','Null kontrolu yapmadan dolasabilirsin.'],
 ['GetBpm / GetCurrentBar / GetCurrentBeat / GetIntensity','Return 0 before a value exists.','Deger olusmadan once 0 dondurur.','Use with IsPlaying if 0 has gameplay meaning.','0 gameplayde anlamliysa IsPlaying ile birlikte kullan.']
];

const conductorEventRows = [
 ['StateChanged(string oldState, string newState)','Raised after the conductor changes state.','Conductor state degistirdikten sonra tetiklenir.','Subscribe from a serialized conductor reference or BST.GetConductor().','Serialize conductor referansindan veya BST.GetConductor() ile abone ol.'],
 ['TrackChanged(string trackName)','Raised when the active track changes.','Aktif track degisince tetiklenir.','Good for debug UI or visual sync.','Debug UI veya gorsel senkron icin iyi.'],
 ['Bar() / Beat(int beat)','Raised by the conductor musical clock.','Conductor muzik saati tarafindan tetiklenir.','Use for lightweight timing reactions, not heavy gameplay work.','Hafif timing tepkileri icin kullan, agir gameplay isi icin degil.'],
 ['HasPendingStateTransition / GetPendingStateName / HasPendingTrackTransition / GetPendingTrackIndex / HasPendingSectionTransition','Expose queued musical transitions.','Siradaki muzik gecisleri gosterir.','Use these in debug UI to disable duplicate buttons while a state, track or section handoff is waiting for its musical boundary.','Bir state, track veya section handoff muzik sinirini beklerken debug UI butonlarini kilitlemek icin kullan.'],
 ['IsDucking / DuckingMultiplier','Expose current ducking state.','Mevcut ducking durumunu gosterir.','Use for debug displays or audio UI.','Debug gostergeleri veya ses UI icin kullan.']
];

const apiReferenceRows = [
 ['GetConductor()','Returns or creates the active conductor through the manager.','Manager uzerinden aktif conductoru dondurur veya olusturur.','Use when you need direct conductor events or advanced status.','Dogrudan conductor eventleri veya ileri durum gerekiyorsa kullan.'],
 ['HasConductor()','Checks whether an active conductor already exists.','Aktif conductor zaten var mi kontrol eder.','Use before optional debug UI.','Opsiyonel debug UI oncesi kullan.'],
 ['StartMusic(float fadeDuration = -1)','Starts the current/auto/default state.','Mevcut/auto/varsayilan statei baslatir.','-1 inherits startFadeDuration.','-1 startFadeDuration miras alir.'],
 ['StartMusic(string stateName, int startTrackIndex = -1, float fadeDuration = -1)','Starts a named state and optional track.','Isimli statei ve opsiyonel tracki baslatir.','Null/empty state falls back through current, autoStartState, then first profile state.','Null/bos state mevcut, autoStartState, sonra ilk profile state sirasiyla duser.'],
 ['StartMusic(string stateName, int startTrackIndex, int startSectionIndex, float fadeDuration = -1)','Starts a named state, track and section.','Isimli state, track ve section baslatir.','Use for section-aware cues that should skip or enter a specific phrase.','Belirli cumleye girmesi gereken section-aware cue icin kullan.'],
 ['StopMusic(float fadeDuration = -1)','Stops playback with a fade.','Playbacki fade ile durdurur.','-1 inherits stopFadeDuration; 0 stops immediately.','-1 stopFadeDuration miras alir; 0 aninda durdurur.'],
 ['SetMusicState / TrySetMusicState','Switches to a state with timing, crossfade, zone and start target options.','Timing, crossfade, bolge ve baslangic hedefi secenekleriyle state degistirir.','Try returns false when the request is invalid or cannot play.','Try, istek gecersizse veya calinamiyorsa false dondurur.'],
 ['SetMusicStateImmediate / SetMusicStateQuantized','Convenience wrappers for immediate or next-bar state changes.','Anlik veya sonraki-bar state degisimi icin kisa yollar.','Both still accept crossfade and start track/section overrides.','Ikisi de crossfade ve start track/section override alir.'],
 ['AddLayer / AddLayers','Adds one or more manual layers.','Bir veya daha fazla manuel layer ekler.','Use fade overrides and addMode to decide ownership.','Sahiplik icin fade override ve addMode kullan.'],
 ['RemoveLayer / ClearLayers','Removes manual layer ownership.','Manuel layer sahipligini kaldirir.','Intensity or zones may still keep their own layers active.','Intensity veya bolgeler kendi layerlarini aktif tutabilir.'],
 ['IsLayerActive / GetAvailableLayers / GetActiveLayers / TryGetLayerFadeDurations','Reads layer state and authored fade data.','Layer durumunu ve yazilmis fade verisini okur.','Use for UI and safe tool panels.','UI ve guvenli tool panelleri icin kullan.'],
 ['LockCurrentTrack / UnlockCurrentTrack / IsTrackLocked','Controls manual track lock.','Manuel track kilidini kontrol eder.','Zone lock is tracked separately; either one makes the track locked.','Zone lock ayri izlenir; ikisinden biri tracki kilitli yapar.'],
 ['JumpToTrack(int trackIndex, float crossfadeDuration = -1, bool quantize = true, int startSectionIndex = -1)','Moves inside the current state playlist.','Mevcut state playlisti icinde hareket eder.','Bool quantize means next bar; jumping to the current track with a section index routes to a section jump.','Bool quantize sonraki bar demektir; mevcut tracke section index ile atlamak section jumpa yonlendirir.'],
 ['JumpToTrack(int trackIndex, float crossfadeDuration, BSTTransitionMode transitionMode, int startSectionIndex = -1)','Moves to another track with full transition-mode timing.','Tam transition-mode zamanlamasi ile baska tracke gecer.','OnSectionEnd and OnNextTransitionPoint wait on the current section track before the track handoff.','OnSectionEnd ve OnNextTransitionPoint track handoff oncesi mevcut section tracki bekler.'],
 ['GetCurrentTrackIndex / GetCurrentTrackName / GetCurrentStateTrackNames','Reads current-state track info.','Mevcut state track bilgisini okur.','Use to build runtime track pickers.','Runtime track secicileri yapmak icin kullan.'],
 ['GetCurrentStateTrackSectionNames / GetCurrentStateTrackEntrySectionIndex','Reads section names and entry section for a track.','Bir trackin section adlarini ve entry sectionini okur.','Useful for panel start-section controls.','Panel start-section kontrolleri icin kullanisli.'],
 ['PushStateRequest / TryPushStateRequest / RemoveStateRequest / ClearStateRequests / GetActiveStateRequests','Priority request arbitration.','Oncelikli request onceliklendirmesi.','Requests do not carry crossfade overrides; they carry id, state, priority, zone force and quantize.','Requestler crossfade override tasimaz; id, state, priority, zone force ve quantize tasir.'],
 ['JumpToSection / JumpToSectionByName','Moves within section-based tracks.','Section tabanli trackler icinde hareket eder.','Quantized jumps wait for valid exits; current-section requests do not restart the phrase.','Quantize jump gecerli cikisi bekler; mevcut section istegi cumleyi restart etmez.'],
 ['GetCurrentSectionNames / GetCurrentSectionInfo / GetCurrentSectionIndex / GetCurrentSectionBar / GetCurrentSectionName / GetSectionLoopCount','Reads section runtime state.','Section runtime durumunu okur.','Use section-local bar for valid exit authoring checks.','Valid exit authoring kontrolu icin section-local bar kullan.'],
 ...
 ['SetCustomFlag / GetCustomFlag / ClearCustomFlags','Sets boolean flags for branch conditions.','Branch kosullari icin boolean flag ayarlar.','Keep flag names stable like API constants.','Flag isimlerini API sabiti gibi sabit tut.'],
 ['SetIntensity(float intensity, bool quantize = true, bool smooth = false, float smoothSpeed = 0, BSTSmoothEasing easingType = EaseInOut, float layerFadeOverride = -1)','Sets 0-1 intensity.','0-1 intensity ayarlar.','Smooth moves the numeric value; layerFadeOverride controls the audible layer-volume fade separately.','Smooth sayisal degeri hareket ettirir; layerFadeOverride duyulan layer-volume fadeini ayri kontrol eder.'],
 ['GetIntensity / GetBpm / GetCurrentBar / GetCurrentBeat / IsPlaying','Timing and status getters.','Zamanlama ve durum getterlari.','Safe before playback; see fallback values.','Playback oncesi guvenli; fallback degerlerine bak.'],
 ['PlayStinger(AudioClip clip, bool quantize = true, float fadeIn = -1) / PlayStinger(string cueName)','Plays direct or profile stingers.','Dogrudan veya profil stingeri calar.','-1 fadeIn inherits defaultStingerFadeIn.','-1 fadeIn defaultStingerFadeIn miras alir.'],
 ['StartDucking / StopDucking / SetDuckLayers / GetDuckLayers / SetDuckStingers / GetDuckStingers','Controls dialogue/key-audio ducking.','Diyalog/onemli ses duckingini kontrol eder.','-1 fadeTime inherits ducking fade defaults; duckStingers decides whether one-shots dip too.','-1 fadeTime ducking fade varsayilanlarini miras alir; duckStingers tek seferlik vurgularin da kisilip kisilmayacagini belirler.']
];
...
${call('note','Useful creation paths','All helpers live on the real Unity menus: <code>GameObject > BeatSync Toolkit > BST Conductor / BST Music Zone / BST Debug HUD</code>, and <code>Assets > Create > BeatSync Toolkit > Music Profile / Intensity Profile</code>.')}`,
...
${call('note','Kullanışlı oluşturma yolları','Tüm helperlar gerçek Unity menülerinde: <code>GameObject > BeatSync Toolkit > BST Conductor / BST Music Zone / BST Debug HUD</code> ve <code>Assets > Create > BeatSync Toolkit > Music Profile / Intensity Profile</code>.')}`},
...
${tbl(['Folder','What is inside','Ships in build?'],[
 ...
 ['Documentation','This offline HTML guide.','No.']
])}
...
${settingRows([
 ...
 ['Documentation','Offline HTML documentation.','Çevrimdışı HTML dokümantasyonu.','Open index.html in any browser; no internet needed.','index.html\'i herhangi bir tarayıcıda aç; internet gerekmez.']
], 'en')}`,
...
${tbl(['Klasör','İçinde ne var','Build’e girer mi?'],[
 ...
 ['Documentation','Bu çevrimdışı HTML kılavuzu.','Hayır.']
])}
...
${settingRows([
 ...
 ['OnSectionEnd','Waits for the current section to finish.','Mevcut sectionin bitmesini bekler.','With fades, the base/section bed can pre-roll so the logical change stays at section end while the audio overlaps.','Fade varsa base/section bed pre-roll yapabilir; logical degisim section sonunda kalirken audio overlap alinir.'],
...
<p>The Control Panel's States tab is a no-code state lab. Fill <code>stateSlots</code> with your real state names, choose the transition mode, test <code>forceStateWhileInZone</code>, choose whether the state persists after leaving zones, and step through start track/section targets.</p>
...
<p>Control Panel'in States tabı kodsuz state laboratuvarıdır. <code>stateSlots</code> içine gerçek state isimlerini yaz, transition mode seç, <code>forceStateWhileInZone</code> davranışını test et, state'in zone sonrası kalıp kalmayacağını seç ve start track/section hedeflerini dene.</p>
...
${settingRows([
 ['State changes','Use BSTTransitionMode plus crossfadeDuration.','BSTTransitionMode ve crossfadeDuration kullanir.','Choose the musical boundary and fade length separately. In the panel, State Fade maps to this value.','Muzikal siniri ve fade uzunlugunu ayri sec. Panelde State Fade bu degere denk gelir.'],
 ['Track jumps','Use bool quantize for next-bar jumps or BSTTransitionMode for section-aware jumps.','Next-bar jump icin bool quantize, section-aware jump icin BSTTransitionMode kullanir.','Use OnSectionEnd for phrase-end jumps or OnNextTransitionPoint when a section track has valid exit bars. In the panel, Track Fade maps to crossfadeDuration.','Cumle sonunda atlamak icin OnSectionEnd, valid exit bar varsa OnNextTransitionPoint kullan. Panelde Track Fade crossfadeDuration degerine denk gelir.'],
...
${call('note','Ayni-state track jumplari section-aware olabilir','JumpToTrack icin BSTTransitionMode overloadunu veya ayni state uzerinde startTrackIndex ile SetMusicState kullan. OnSectionEnd mevcut sectionin bitmesini bekler; OnNextTransitionPoint hedef tracke gecmeden once validExitBars uzerinden cikabilir.')}
${call('note','Section-end pre-roll','Fade olan OnSectionEnd degisiminde toolkit hedef base/section bed sesini erken baslatabilir ve hedef track stemlerini ayni DSP zaman cizgisinde hazirlayabilir. Gorunen state, track, section, bar clock, layer ownership ve eventler yine gercek section sonunda commit edilir.')}
${call('tip','Panelde section-end fade testi','Runtime Control Panel icinde States sekmesinde State Fade, Tracks sekmesinde Track Fade vardir. -1 Conductor defaultCrossfadeDuration miras alir, 0 sert boundary kesiti yapar, 2.00 gibi acik bir deger iki fazli OnSectionEnd pre-roll overlapini duymani saglar.')}
${ex('Ornek: sectionli full mixte basla, Track Jump Mode degerini Section End yap, Track Fade degerini 2.00 yap ve stemli bir tracke jump et. Eski section logical olarak gercek sonunda biter; hedef bed ve hazirlanan stemler boundary oncesinde overlap alabilir.')}
...
<li><b>Fill the quick buttons:</b> add your real state names to <code>stateSlots</code>, and any global layer names to <code>fallbackLayers</code>.</li>
...
${call('tip','Development only','Both the panel and the Debug HUD are testing tools. Remove or disable them before shipping, or gate the HUD behind a debug flag for QA builds.')}
<h3>Debug HUD</h3>
<p>Create it from <code>GameObject > BeatSync Toolkit > BST Debug HUD</code>. If no conductor is assigned, it finds the active one automatically. It shows current state, bar/beat, BPM, intensity, active layers and the top music zone. If no conductor is available it displays <code>NO CONDUCTOR</code>. <code>refreshInterval</code> controls how often the UI updates.</p>
<p>The HUD fields are normal Unity UI references. Assign <code>stateText</code>, <code>barBeatText</code>, <code>bpmText</code>, <code>intensityText</code>, <code>intensitySlider</code>, <code>layersText</code> and <code>activeZoneText</code> to decide which debug values are visible. The color fields map common state names to quick-read colors; <code>defaultStateColor</code> covers custom states such as Boss, Menu or Chase.</p>
<h3>Settings reference - Debug HUD</h3>
${settingRows(debugHudSettings, 'en')}
...
${settingRows([
 ...
 ['States tab','Calls SetMusicState with transition mode, State Fade, force, persist, start track and section options.','transition mode, State Fade, force, persist, start track ve section secenekleriyle SetMusicState cagirir.','State Fade maps to crossfadeDuration: -1 inherits the conductor default, 0 cuts, explicit seconds test special moments.','State Fade crossfadeDuration degerine denk gelir: -1 conductor defaultunu miras alir, 0 keser, acik saniye ozel anlari test eder.'],
 ...
 ['Tracks tab','Calls JumpToTrack with the selected transition mode, Track Fade and start section option.','Secili transition mode, Track Fade ve start section secenegiyle JumpToTrack cagirir.','Track Fade maps to crossfadeDuration; use OnSectionEnd plus an explicit fade to test two-phase pre-roll.','Track Fade crossfadeDuration degerine denk gelir; iki fazli pre-roll testi icin OnSectionEnd ve acik fade kullan.'],
...
${settingRows([
 ...
 ['States','Quick state buttons plus force/persist, fade, and start track/section controls.','Hizli state butonlari, force/persist, fade ve start track/section kontrolleri.','Use it to test SetMusicState timing and crossfade behavior before gameplay code exists.','Gameplay kodu yokken SetMusicState zamanlama ve crossfade davranisini test etmek icin kullan.'],
...
<li><b>Hızlı butonları doldur:</b> gerçek state adlarını <code>stateSlots</code>'a, global layer adlarını <code>fallbackLayers</code>'a ekle.</li>
...
${call('tip','Yalnızca geliştirme','Hem panel hem Debug HUD test aracıdır. Yayından önce kaldır veya devre dışı bırak, ya da QA build’leri için HUD’u bir debug bayrağının arkasına al.')}
<h3>Debug HUD</h3>
<p><code>GameObject > BeatSync Toolkit > BST Debug HUD</code> yolundan oluştur. Conductor atanmadıysa aktif olanı otomatik bulur. Mevcut state, bar/beat, BPM, intensity, aktif layerlar ve top music zone değerlerini gösterir. Conductor yoksa <code>NO CONDUCTOR</code> yazar. <code>refreshInterval</code> UI'ın ne kadar sık güncelleneceğini belirler.</p>
<p>HUD alanları normal Unity UI referanslarıdır. Hangi debug değerleri görünecekse <code>stateText</code>, <code>barBeatText</code>, <code>bpmText</code>, <code>intensityText</code>, <code>intensitySlider</code>, <code>layersText</code> ve <code>activeZoneText</code> alanlarını ata. Renk alanları yaygın state adlarını hızlı okunan renklere bağlar; <code>defaultStateColor</code> Boss, Menu veya Chase gibi özel stateleri kapsar.</p>
<h3>Ayar referansı - Debug HUD</h3>
${settingRows(debugHudSettings, 'tr')}
